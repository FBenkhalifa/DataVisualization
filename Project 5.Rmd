...
mydata %>% 
  ...
  as.matrix() %>% 
  stargazer(., title = "Mean values", style = "latex")
```
...
The estimation results of this simple model are displayed in the first column of table X.
...
model_metrics %>% 
  pull(fit) %>% 
  exec(stargazer, ., title = "Results", align = TRUE, type = "latex",
...
```{r, results = "asis"}
model_metrics$fit[[8]] %>% anova(test = "Chisq") %>% as.matrix %>% stargazer(., title = "Chi-Squared for Model (1)", type = "latex")
model_metrics$fit[[1]] %>% anova(test = "Chisq")%>% as.matrix %>% stargazer(., title = "Chi-Squared for Model (8)", type = "latex")
